...
**ПРАВИЛО: ВСЕ файлы (планы, задачи, анализы, ревью, спецификации) писать ТОЛЬКО в корень репозитория:**

```
✅ ПРАВИЛЬНО:  <корень репозитория>/MemoryBank/...
✅ ПРАВИЛЬНО:  <корень репозитория>/Doc/...
✅ ПРАВИЛЬНО:  <корень репозитория>/modules/...
...
**Конкретно для MemoryBank и Doc:**
- Планы, таски, идеи, ревью, анализы → `MemoryBank/` (основной)
- Документация → `Doc/` (основной)
- Сессии → `MemoryBank/sessions/` (основной)
- НИКОГДА не создавать эти файлы в `.claude/worktrees/*/`

---

### 🌿 Две ветки — параллельная жизнь (не объединять!)

**Всегда рассматривать 2 ветки и 2 линии рассуждений:**

| Ветка   | Платформа | Сборка        | fft_func      |
|---------|-----------|---------------|---------------|
| **main**   | Linux, AMD GPU | Debian-Radeon9070 и др. | ROCm/hipFFT   |
| **nvidia** | Windows, NVIDIA GPU | Ninja + MSVC (VS 2026) | OpenCL/clFFT |

- **Пишем под Windows** → ветка `nvidia`, там и собираем.
- **Правим под Linux** → ветка `main`, там и собираем.

Ветки **не планируется объединять** — параллельное развитие.
...
> 🚨 **СИНЬОРЫ И АГЕНТЫ**: Любые результаты (анализ, ревью, планы, аудит) — записывать ТОЛЬКО в основной `MemoryBank/`, никогда в worktree! Агенты могут читать код из worktree, но ПИСАТЬ только в основной каталог проекта.
...
### OpenCL / ROCm Backend
🔑 **Правило выбора FFT-библиотеки по платформе**:

| Платформа | FFT библиотека | Тесты | Примечание |
|-----------|---------------|-------|------------|
| **AMD GPU** | ROCm / hipFFT | ROCm-тесты | clFFT НЕ работает на RDNA4+ (gfx1201) |
| **NVIDIA GPU** | OpenCL / clFFT | OpenCL-тесты | ROCm недоступен |

- ⚠️ **На AMD НЕ тестируем clFFT** — библиотека мертва, не поддерживает новые архитектуры
- ⚠️ **На NVIDIA НЕ тестируем ROCm** — недоступен
- Методы с параметром `backend_type` или флагами выбора реализации
- Проверяй код на наличие `ENABLE_ROCM`, `BACKEND_*` флагов
