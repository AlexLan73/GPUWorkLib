...
# ============================================================================
# PipelineBase — Template Method (GoF) + Abstract Class
# ============================================================================
# Рефакторинг: выносим общую логику run_pipeline_a/b в базовый класс.
# PipelineRunner остаётся публичным API (Façade) и делегирует сюда.
# ============================================================================
...
class PipelineBase(ABC):
    """Абстрактный pipeline — общий код вынесен из run_pipeline_a/b.

    Template Method (GoF):
        run() — неизменный скелет (stats → preprocess → FFT → peaks → save)
        _preprocess() — abstract hook (разная логика для A и B)

    Подклассы:
        PipelineA — фазовая коррекция W_phase + GEMM
        PipelineB — Farrow alignment + W_sum + GEMM
    """

    @property
    @abstractmethod
    def name(self) -> str:
        """Имя pipeline ("A" или "B")."""
        ...

    def run(self, scenario: dict, steer_theta: float,
            config: Optional[PipelineConfig] = None,
            save_dir: str = "",
            steer_freq: float = 0.0) -> PipelineResult:
        """Общий скелет pipeline (Template Method).

        Шаги:
          0. Input stats
          _preprocess() → (W, X, result) — abstract hook
          3. Window + FFT
          4. Peak detection
          5. Save

        Args:
            scenario:    dict от ScenarioBuilder.build()
            steer_theta: угол наведения (градусы)
            config:      PipelineConfig (checkpoint'ы)
            save_dir:    директория для записи (пустая = не писать)
            steer_freq:  частота для фазовой коррекции (только Pipeline A)
        ...
        # Step 0: Input stats
        result.stats_input = compute_matrix_stats(S, "input")
        if cfg.save_input and save_dir:
            np.save(os.path.join(save_dir, "S_raw.npy"), S)

        # Hook: специфичная предобработка
        W, X, result = self._preprocess(scenario, steer_theta, steer_freq,
                                         result, cfg, save_dir)
        result.W = W
        result.X_gemm = X
        result.stats_gemm = compute_matrix_stats(X, "gemm")
        if cfg.save_gemm and save_dir:
            np.save(os.path.join(save_dir, "X_gemm.npy"), X)

        # Step 3: Window + FFT (общий для A и B)
        nFFT = next_pow2_x2(n_samples)
        win = hamming_window(n_samples)
        spectrum = np.zeros((n_ant, nFFT), dtype=np.complex64)
        magnitudes = np.zeros((n_ant, nFFT), dtype=np.float32)
        ...
        if cfg.save_spectrum and save_dir:
            np.save(os.path.join(save_dir, "spectrum.npy"), spectrum)
            np.save(os.path.join(save_dir, "magnitudes.npy"), magnitudes)

        # Step 4: Peak detection (общий)
        result.peaks = find_peaks_per_beam(magnitudes, result.freq_axis)

        # Step 5: Save stats + results (общий)
        if cfg.save_stats and save_dir:
            self._save_stats(result, save_dir)
        if cfg.save_results and save_dir:
    ...
    @abstractmethod
    def _preprocess(self, scenario: dict, steer_theta: float,
                    steer_freq: float, result: PipelineResult,
                    cfg: PipelineConfig,
                    save_dir: str):
        """Специфичная предобработка (фазовая или Farrow).

        Returns:
            (W, X, result) — матрица весов, выход GEMM, обновлённый result
        """
        ...

    @staticmethod
    def _save_stats(result: PipelineResult, save_dir: str):
        stats_data = {
            'pipeline': result.pipeline_name,
            'steer_theta_deg': result.steer_theta_deg,
            'steer_freq_hz': result.steer_freq_hz,
            'nFFT': result.nFFT,
        }
        if result.stats_input:
            stats_data['stats_input'] = [s.to_dict() for s in result.stats_input]
        if result.stats_aligned:
            stats_data['stats_aligned'] = [s.to_dict() for s in result.stats_aligned]
        if result.stats_gemm:
            stats_data['stats_gemm'] = [s.to_dict() for s in result.stats_gemm]
        if result.stats_spectrum:
            stats_data['stats_spectrum'] = [s.to_dict() for s in result.stats_spectrum]
        with open(os.path.join(save_dir, "stats.json"), 'w') as f:
            json.dump(stats_data, f, indent=2, default=str)

    @staticmethod
    def _save_results(result: PipelineResult, save_dir: str):
        results_data = {
            'pipeline': result.pipeline_name,
            'peaks': [[p.to_dict() for p in beam] for beam in (result.peaks or [])],
        }
...
class PipelineA(PipelineBase):
    """Pipeline A: фазовая коррекция W_phase + GEMM.

    W_phase[beam][ant] = (1/√N) · exp(-j·2π·f·τ_ant)
    """
    ...
    def _preprocess(self, scenario, steer_theta, steer_freq, result, cfg, save_dir):
        S = scenario['S']
        array: ULAGeometry = scenario['array']
        n_ant = array.n_ant
...
class PipelineB(PipelineBase):
    """Pipeline B: Farrow alignment + W_sum + GEMM.

    W_sum[beam][ant] = 1/√N — когерентное суммирование.
    """
    ...
    def _preprocess(self, scenario, steer_theta, steer_freq, result, cfg, save_dir):
        ...
        # Farrow: компенсировать задержки антенн
        farrow = FarrowDelay()
        delays_s = array.compute_delays(steer_theta)
        S_aligned = farrow.compensate_seconds(S, delays_s, fs)
        ...
        result.S_aligned = S_aligned
        result.stats_aligned = compute_matrix_stats(S_aligned, "aligned")
        if cfg.save_aligned and save_dir:
            np.save(os.path.join(save_dir, "S_aligned.npy"), S_aligned)

        # W_sum: равное когерентное суммирование
        inv_sqrt_n = 1.0 / np.sqrt(n_ant)
        W = np.full((n_ant, n_ant), inv_sqrt_n, dtype=np.complex64)
