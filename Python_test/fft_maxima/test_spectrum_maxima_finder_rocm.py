...
rocm_only = pytest.mark.skipif(not HAS_ROCM_MAXIMA,
                                reason="SpectrumMaximaFinderROCm not available")
...
class TestNumPyReference:
    ...
    def test_freq_resolution_formula(self):
        ...
        nFFT = next_pow2(N)
        freq_res = fs / nFFT
        assert freq_res == pytest.approx(fs / nFFT, rel=1e-9)
    ...
    def test_single_tone_magnitude(self):
        ...
        assert peak_mag == pytest.approx(N, rel=1e-4)
...
class TestSpectrumMaximaFinderROCm:
    ...
    def finder(self):
        ...
        return gpuworklib.SpectrumMaximaFinderROCm(ctx)

    # ── process() — ONE_PEAK ─────────────────────────────────────────────────

    def test_process_single_beam_peak_freq(self, finder):
    ...
    def test_process_multi_beam_list(self, finder):
        ...
        results = finder.process(data, n_point=N, sample_rate=fs,
        ...
        for i, (r, k) in enumerate(zip(results, ks)):
            expected_hz = k * fs / next_pow2(N)
            assert r["freq_hz"] == pytest.approx(expected_hz, rel=0.05), \
                f"Beam {i}: expected {expected_hz:.0f}, got {r['freq_hz']:.0f}"

    def test_process_result_fields(self, finder):
        """process(): результат содержит все ожидаемые поля."""
        sig = make_single_tone(128, 10)
        result = finder.process(sig, n_point=128, sample_rate=1e6)
    ...
    def test_find_all_maxima_two_tones(self, finder):
        ...
        result = finder.find_all_maxima(spec, nFFT=next_pow2(N), sample_rate=fs)

        assert isinstance(result, dict), "Single beam → dict"
        assert result["num_maxima"] >= 2, \
            f"Expected >= 2 maxima, got {result['num_maxima']}"

    def test_find_all_maxima_opencl_compat_format(self, finder):
        ...
        result = finder.find_all_maxima(spec, nFFT=N, sample_rate=1e6)
    ...
    def test_find_all_maxima_peak_frequency(self, finder):
        ...
        result = finder.find_all_maxima(spec, nFFT=N, sample_rate=fs)
