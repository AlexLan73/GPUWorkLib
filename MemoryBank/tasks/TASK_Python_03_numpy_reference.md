# TASK Python-03: strategies/numpy_reference.py — NumpyReference

**Статус**: 🔲 TODO
**Приоритет**: 🟡 ВЫСОКИЙ — нужен для TASK_Python_05
**Файл**: `Python_test/strategies/numpy_reference.py` (создать новый)
...
Создать класс `NumpyReference` — CPU-эталон для каждого шага pipeline `strategies`.

Класс вычисляет всё заранее при инициализации: GEMM, Hamming, FFT, статистику.
В тестах используется для сравнения с GPU-результатами.

**Information Expert (GRASP)**: NumpyReference знает свои данные и умеет считать статистику сам.

---

## 📖 Что прочитать перед реализацией

- `MemoryBank/specs/python_test_refactoring.md` — раздел "Типы данных" и "Диаграмма pipeline"
- `modules/strategies/include/result_types.hpp` — структуры StatisticsResult, AntennaResult
- `modules/statistics/include/statistics_types.hpp` — StatisticsResult поля

---

## ⚠️ Важные факты о типах

Все вычисления в C++ используют `float32` / `complex<float>`. NumPy reference должен это повторять:

```
S_ref  : np.complex64  [n_ant, n_samples]
W_ref  : np.complex64  [n_ant, n_ant]
X_ref  : np.complex64  [n_ant, n_samples]  ← W_ref @ S_ref
spec_ref: np.complex64 [n_ant, n_fft]      ← FFT результат
mag_ref : np.float32   [n_ant, n_fft]      ← |spec_ref|
hamm   : np.float32    [n_samples]         ← окно Хемминга
```

Статистика C++ (StatisticsResult) для комплексных данных:
```
mean         : complex<float>  ← комплексное среднее
variance     : float           ← Var(|z|)   — от МАГНИТУД!
std_dev      : float           ← sqrt(Var(|z|))
...
class NumpyReference:
    ...
    def __init__(self,
        ...
        self.input_stats: list[StatsRef] = self._compute_stats(self.S_ref)
        ...
        self.gemm_stats: list[StatsRef] = self._compute_stats(self.X_ref)

        # --- STEP 4: Hamming + zero-pad + FFT ---
        # Окно Хемминга (float32, как в GPU)
        ...
        self.fft_stats: list[StatsRef] = self._compute_stats(self.mag_ref)
...
1. `NumpyReference.__init__` принимает `S`, `W`, `fs`, `f0`, `n_fft`
2. Поля `S_ref`, `W_ref`, `X_ref`, `spec_ref`, `mag_ref` — dtype `complex64`/`float32`
3. `input_stats`, `gemm_stats`, `fft_stats` — `list[StatsRef]` длиной `n_ant`
4. `StatsRef.mean` — `complex64` (даже если данные float)
5. `StatsRef.variance/std_dev/mean_magnitude` — float32, вычисленные от `|arr|`
6. `expected_peak_bin = round(f0 * n_fft / fs)` — правильная формула
7. `compute_dynamic_range_db()` → `np.ndarray[float32]` shape `[n_ant]`
8. При `W = I` (Identity): `X_ref ≈ S_ref` с точностью float32

---

## ❌ Что НЕ делать

- НЕ хранить промежуточные данные (padded) как поля класса — только финальные результаты
- НЕ использовать float64 в выходных данных (только для внутренних вычислений)
- НЕ добавлять логику сравнения — это задача `DataValidator`
- НЕ делать `NumpyReference` изменяемым после создания (`frozen=True` для `StatsRef`)
