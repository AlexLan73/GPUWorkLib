...
/**
 * @file iir_kernels.hpp
 * @brief OpenCL kernel sources for IIR biquad cascade filter
 *
 * Strategy: one work-item per channel, sequential loop over samples.
 * All biquad sections processed in a single kernel call (loop over sections).
 * Direct Form II Transposed (numerically stable).
 *
 * Complex signal: float2 (x=Re, y=Im), real biquad coefficients.
 *
...
/**
 * @brief Get IIR biquad cascade kernel source (OpenCL)
 *
 * Kernel: iir_biquad_cascade_cf32
 *   - NDRange 1D: global_size = {channels}
 *   - Each work-item: ALL samples for one channel, ALL biquad sections
 *   - SOS matrix: [num_sections * 5] = {b0, b1, b2, a1, a2} per section
 *
 * Parallelism: across channels only (data dependency within channel)
 * For N sections: output of section k -> input of section k+1
 * Uses ping-pong between input→output for first section, output→output for rest
 *
 * @return Pointer to kernel source string
 */
inline const char* GetIirBiquadCascadeSource_opencl() {
  return R"CL(
