...
static const char* LFM_KERNEL_SOURCE = R"CL(
/**
 * @brief LFM (chirp) генерация
 *
 * s(t) = amplitude * exp(j * (pi * chirp_rate * t^2 + 2*pi*f_start*t))
 *
 * Все лучи генерируют одинаковый LFM сигнал (для multi-beam).
 */
...
void LfmGenerator::CompileKernel() {
    cl_int err;
    size_t source_len = strlen(LFM_KERNEL_SOURCE);

    program_ = clCreateProgramWithSource(context_, 1, &LFM_KERNEL_SOURCE, &source_len, &err);
    if (err != CL_SUCCESS) {
