 ...
 * };
 *
 * // Вызов Process с новым API
 * auto results = finder.Process(cpu_input, PeakSearchMode::ONE_PEAK, DriverType::OPENCL);
 * @endcode
 */
template<typename T>
struct InputData {
 ...
 *
 * @deprecated Используйте поля в InputData<T> вместо отдельной структуры.
 *             Оставлено для внутреннего использования.
 *
 * Пример:
 * @code
