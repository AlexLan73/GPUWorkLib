 ...
 *
 * Структура userdata (32 bytes header):
 *   [0]:  beam_count (uint)
 *   [1]:  count_points (uint)
 *   [2]:  nFFT (uint)
 *   [3-7]: padding
 *   [32+]: входные данные (beam_count * count_points * complex<float>)
 *
 * Callback вызывается clFFT для каждого элемента FFT входа.
 * Если позиция < count_points — читаем данные, иначе возвращаем 0 (zero-padding).
 */
                      ...
                      __global void* userdata)
{
    ...
    if (pos < count_points && beam_id < beam_count) {
        // Смещение данных: 32 байта заголовок = 4 float2 (8 uint)
