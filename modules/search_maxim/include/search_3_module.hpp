 ...
 * 
 * // 2. Создание модуля
 * Search3Params params(5, 1000, 512, 3); // 5 лучей, 1000 точек, 512 FFT out, 3 пика
 * auto module = std::make_shared<Search3Module>(&gpu.GetBackend(), params);
 * module->Initialize();
 * 
 ...
 * ```
 */
class Search3Module : public IComputeModule {
public:
    // ═══════════════════════════════════════════════════════════════════════
     ...
     */
    explicit Search3Module(IBackend* backend, const Search3Params& params);
    
    /**
     * @brief Деструктор (очищает kernels, планы FFT, буферы)
     */
    ~Search3Module() override;
    
    // Запрет копирования, поддержка перемещения
    Search3Module(const Search3Module&) = delete;
    Search3Module& operator=(const Search3Module&) = delete;
    Search3Module(Search3Module&&) noexcept;
    Search3Module& operator=(Search3Module&&) noexcept;
    ...
    std::string GetName() const override { return "Search3"; }
     ...
     * @return Search3FFTResult с найденными максимумами для каждого луча
     */
    /**
     ...
     * @return Search3FFTResult с найденными максимумами для каждого луча
     */
    Search3FFTResult ProcessNew(std::shared_ptr<GPUBuffer<std::complex<float>>> input_signal);
     ...
     * @return Search3FFTResult с найденными максимумами для каждого луча
     */
    Search3FFTResult ProcessNew(cl_mem input_signal);
     ...
     */
    void UpdateParams(const Search3Params& params);
...
private:
     ...
     */
    Search3FFTResult ProcessSingleBatch(cl_mem input_signal);
     ...
     * @return Результаты для всех лучей
     */
    Search3FFTResult ProcessMultiBatch(cl_mem input_signal);
    ...
    Search3Params params_;           ///< Параметры обработки
    ...
    std::shared_ptr<GPUBuffer<std::complex<float>>> buffer_fft_input_;
    std::shared_ptr<GPUBuffer<std::complex<float>>> buffer_fft_output_;
    
    // Кэшированные буферы для batch обработки
    std::shared_ptr<GPUBuffer<std::complex<float>>> batch_fft_input_;
    std::shared_ptr<GPUBuffer<std::complex<float>>> batch_fft_output_;
    size_t batch_buffers_size_;      ///< Текущий размер буферов (num_beams)
    
    // Буфер для результатов (MaxValue структуры)
    std::shared_ptr<GPUBuffer<float>> buffer_maxima_;
};
