...
Search3Module::Search3Module(IBackend* backend, const Search3Params& params)
    : backend_(backend)
    , params_(params)
    , batch_config_()
    , initialized_(false)
...
Search3Module::~Search3Module() {
    Cleanup();
}

Search3Module::Search3Module(Search3Module&& other) noexcept
...
Search3Module& Search3Module::operator=(Search3Module&& other) noexcept {
    if (this != &other) {
        Cleanup();
...
void Search3Module::Initialize() {
    ...
    // Инициализировать clFFT (clfftInitSetupData - inline в заголовке, дублируем для -O0)
...
size_t Search3Module::CalculateNFFT(size_t count_points) const {
...
bool Search3Module::IsPowerOf2(size_t n) const {
    return n > 0 && (n & (n - 1)) == 0;
}

size_t Search3Module::NextPowerOf2(size_t n) const {
    if (n == 0) return 1;
...
size_t Search3Module::EstimateRequiredMemory() const {
    // Входные данные
...
bool Search3Module::CheckAvailableMemory(size_t required_memory) const {
...
size_t Search3Module::CalculateBatchSize(size_t total_beams) const {
...
void Search3Module::UpdateParams(const Search3Params& params) {
    if (!params.IsValid()) {
        throw std::invalid_argument("Search3Module::UpdateParams: invalid parameters");
    }
