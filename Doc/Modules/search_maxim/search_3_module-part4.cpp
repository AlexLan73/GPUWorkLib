/**
 * @file antenna_module.cpp - ЧАСТЬ 4/4
...
AntennaFFTResult AntennaModule::ProcessMultiBatch(cl_mem input_signal) {
    DRVGPU_LOG_INFO("AntennaModule", "ProcessMultiBatch: start");
    ...
    if (!batch_fft_input_ || batch_buffers_size_ != max_batch) {
        ...
        batch_fft_input_ = mem_mgr.CreateGPUBuffer<std::complex<float>>(buffer_size);
        batch_fft_output_ = mem_mgr.CreateGPUBuffer<std::complex<float>>(buffer_size);
        batch_buffers_size_ = max_batch;
        DRVGPU_LOG_INFO("AntennaModule", "Created batch buffers (size=" + 
    ...
    // Собрать результат
    AntennaFFTResult final_result(total_beams, nFFT_, params_.task_id, params_.module_name);
    final_result.results = std::move(all_results);
    
    DRVGPU_LOG_INFO("AntennaModule", "ProcessMultiBatch: complete");
    return final_result;
}
...
std::vector<BeamFFTResult> AntennaModule::ProcessBatch(
    cl_mem input_signal, size_t start_beam, size_t num_beams) 
{
    ...
    cl_mem fft_input_mem = batch_fft_input_->GetNativeHandle();
    ...
    if (err != CL_SUCCESS) {
        throw std::runtime_error("AntennaModule::ProcessBatch: padding_kernel failed");
    }
    
    // ═══ 2. FFT transform ═══
    cl_mem fft_output_mem = batch_fft_output_->GetNativeHandle();
    ...
    if (status != CLFFT_SUCCESS) {
        throw std::runtime_error("AntennaModule::ProcessBatch: clfftEnqueueTransform failed");
    }
    ...
    err = clEnqueueNDRangeKernel(queue_, post_kernel_, 2, nullptr, global_post,
                                 nullptr, 0, nullptr, nullptr);
    if (err != CL_SUCCESS) {
        throw std::runtime_error("AntennaModule::ProcessBatch: post_kernel failed");
    }
...
std::vector<BeamFFTResult> AntennaModule::FindMaximaOnGPU(size_t num_beams) {
    ...
    if (!buffer_maxima_ || buffer_maxima_->GetSize() != result_buffer_floats) {
        auto& mem_mgr = backend_->GetMemoryManager();
        buffer_maxima_ = mem_mgr.CreateGPUBuffer<float>(result_buffer_floats);
    }
    
    cl_mem maxima_mem = buffer_maxima_->GetNativeHandle();
    ...
    if (err != CL_SUCCESS) {
        throw std::runtime_error("AntennaModule::FindMaximaOnGPU: failed to clear buffer");
    }
    ...
    if (err != CL_SUCCESS) {
        throw std::runtime_error("AntennaModule::FindMaximaOnGPU: failed to read results");
    }
